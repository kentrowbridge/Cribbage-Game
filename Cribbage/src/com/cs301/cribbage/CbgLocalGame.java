...
class CbgLocalGame extends LocalGame{
        ...
        public CbgLocalGame(){
                ...
                state = new CbgState();
                state.setDeck(deck);  
                state.cribOwner = (int) Math.random()*2 + 1;
                gameCycle();
        ...
        private void gameCycle()
        {
                while (!state.getGameOver())
                {
                        ...
                        deck.shuffle();
                        if (state.cribOwner == state.PLAYER_1) state.cribOwner = state.PLAYER_2;
                        else state.cribOwner = state.PLAYER_1;
                }// end while. continue until game over
        }//gameCycle
