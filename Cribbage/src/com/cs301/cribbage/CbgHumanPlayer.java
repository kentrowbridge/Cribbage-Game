...
class CbgHumanPlayer extends GameHumanPlayer implements OnClickListener, Animator{
	...
	private RectF[] cribPos = new RectF[4];//position of cards in crib
	...
	public final void confirm() { 
		...
		if(state.getGameStage() == state.THROW_STAGE){//checks if game is in throw stage
			if(isFull(selectedCards)){//checks if there are two selected cards
				action = new CardsToThrow(this, selectedCards);//sets action
				state.setCrib(selectedCards);
				for(int i = 0; i < selectedCards.length;i++){
					//goes through selected cards and deletes those cards from hand
					int cardIdx = indexOfCard(tempHand, selectedCards[i]);
					int length = tempHand.length;
					if (cardIdx >=0 && cardIdx < tempHand.length){//if card pos is valid
						//if card is in hand, remove that card
						tempHand[cardIdx] = null;
					}
				}
			}
		}else if(state.getGameStage() == state.PEG_STAGE){//checks if game is in peg stage
			//TODO make functionality for sending card to table
	...
	public void onClick(View v) {
		if(v.getId() == confirm.getId()){
			if(state.getGameStage() == state.THROW_STAGE){
				if(isFull(selectedCards)){//if to Throw is completely full, confirm cards
					confirm();
				}else{return;}//otherwise, ignore
			}else if(state.getGameStage() == state.PEG_STAGE){
				confirm();
			}
		}else if (v.getId() == R.id.quitButton){//if quit button is selected, quit
			System.exit(0);
		}
	}
	...
	public void onTouch(MotionEvent event) {
		if(event.getAction() == event.ACTION_DOWN){
			...
			if(state != null && state.getTurn() == state.PLAYER_1){//checks if state is not null, cecks stage
				//and checks whose turn it is
				for(int i = 0; i<6; i++){//goes through players hand
					if(handCardPos[i] != null && handCardPos[i].contains(x, y)){//is not null, and contains touch point
						selectCard(tempHand[i]);//selects card that contains touch point
						break;//after card is selected, return
					}
				}
			}
		}
...
//		}
	...
	private void selectCard(Card c) {
		...
		if(selectedCards[0] == c){
			selectedCards[0] = null;//deselects card
		}else if(selectedCards[1] == c){
			selectedCards[1] = null;//deselects card
...
//		}
	...
	public void tick(Canvas c) {		
		...
		cribPos[0] = new RectF(9*cardWidth, height*2/3, 10*cardWidth, height);
		cribPos[1] = new RectF(9*cardWidth + cardWidth/3, height*2/3, 11*cardWidth, height);
		cribPos[2] = new RectF(9*cardWidth + cardWidth*2/3, height*2/3, 10*cardWidth, height);
		cribPos[3] = new RectF(10*cardWidth, height*2/3, 11*cardWidth, height);
		...
		if(tempHand != null){
			for(int i = 0; i < tempHand.length;i++){
				if(tempHand[i]!=null){
					...
					Paint cover = new Paint(Color.RED);
					c.drawRect(handCardPos[i], cover);//covers up cards left over
				}
			}
		}
		boolean cribNull = false;
		for (Card cards : crib){
			if (cards == null){// fix this, if last element is null returns true
				cribNull = true;
			}
		}
		if(!cribNull){
			for(int i = 0; i < crib.length;i++){
				crib[i].drawOn(c,cribPos[i]);//draws the cards in each position
			}
		}

		if(cardsOnTable.size() >= 0){
			int counter = 0;
			for(Card tableVals : cardsOnTable){
				tableVals.drawOn(c, tableCardPos[counter]);
				counter++;
			}
		}
	}
	...
	private void highLightCard(Canvas c, Rect pos){
		Paint highLight = new Paint(Color.YELLOW);
		highLight.setAlpha(60);
		c.drawRect(pos, highLight);
	}
