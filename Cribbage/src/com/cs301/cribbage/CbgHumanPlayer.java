	...
	public void tick(Canvas c) {
		...
		for(int i = 0; i < tempHand.length;i++){
			if(tempHand[i] != null){//if card is null, dont draw it
				...
				c.drawRect(handCardPos[i], new Paint(Color.BLACK));
			}
		}
		...
		if(selectedCards[0] != null){
			highLight(c, handCardPos[indexOfCard(tempHand, selectedCards[0])]);
		}
		if(selectedCards[1] !=null){
			highLight(c, handCardPos[indexOfCard(tempHand,selectedCards[1])]);
		}
	}
